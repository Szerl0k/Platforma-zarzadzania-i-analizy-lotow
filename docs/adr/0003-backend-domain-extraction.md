...
# ADR-0003: Wydzielenie nowych kontekstów domenowych (Simulations, Telemetry, Tracking) z modułu Flights
...
4. Niestandardowe symulacje lotów (encja `CustomFlightSimulation`).
...
3. **Pełna separacja na dedykowane Bounded Contexts** — precyzyjne wydzielenie trzech nowych modułów: `simulations`, `telemetry`, `tracking`, przy pozostawieniu w `flights` wyłącznie danych operacyjnych.
...
4. **`simulations/` (Domena symulacyjna)**
   * Odpowiedzialność: Tworzenie, przeliczanie i zarządzanie niestandardowymi symulacjami lotów tworzonymi przez użytkowników.
   * Przeniesione encje: `CustomFlightSimulation` (oraz nowa `SimulationStatus`).
...
│   └── services/
│
├── simulations/                   # Nowy moduł symulacji
│   ├── entities/          
│   ├── routes/
│   └── services/
│
