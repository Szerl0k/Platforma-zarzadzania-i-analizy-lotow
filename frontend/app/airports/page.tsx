    ...
    async function loadByExactCode(code: string) {
        ...
        try {
            ...
            if (axios.isAxiosError(err) && err.response?.status === 404) {
                setError(`Nie znaleziono lotniska "${code}" ani w bazie, ani w AeroAPI.`);
            } else {
        ...
        <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
            <div className="container mx-auto px-4 py-12 max-w-4xl">
                ...
                <p className="text-gray-600 mb-8">
                    Wpisz nazwę lub kod ICAO/IATA. Jeśli dany kod nie znajduje się w bazie, użyj
                    przycisku <span className="font-medium">Pobierz dokładny kod</span> — backend
                ...
                <section className="bg-white rounded-lg shadow p-4 mb-6">
                    ...
                    {results.length === 0 && !searchLoading && (
                        <p className="text-sm text-gray-500">
                            {query.trim() ? 'Brak wyników w lokalnej bazie.' : 'Wpisz frazę, aby wyszukać.'}
                        </p>
                    )}
                ...
                {selected && (
                    <section className="bg-white rounded-lg shadow p-4">
                        ...
                        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                            ...
                            <dd className="font-mono text-gray-800">{selected.iataCode ?? '—'}</dd>
                            ...
                            <dd className="font-mono text-gray-800">{selected.city?.countryCode ?? '—'}</dd>
