...
export default function AdminRolesPage() {
    ...
    async function handleCreate(e: FormEvent) {
        ...
        setError(null);
        try {
            await createRole({ name: newName.trim(), description: newDescription.trim() || null });
            setNewName('');
            setNewDescription('');
            await fetchRoles();
        } catch (err) {
    ...
    return (
        <div className="space-y-6">
                                    ...
                                    {editingId === role.id ? (
                                        <div className="border-2 border-ink p-3 space-y-2">
                                            ...
                                            <div className="flex gap-2">
                                                <Button size="sm" onClick={saveEdit}>Zapisz</Button>
                                            ...
                                            <div className="flex items-center justify-between gap-2">
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="font-sans text-ink">{role.name}</span>
                                                        {role.isSystem && <Badge variant="navy">SYSTEM</Badge>}
                                                    </div>
                                                    {role.description && (
                        ...
                        {availableToGrant.length > 0 && (
                            <div className="flex flex-wrap gap-2 items-end">
                                <select
                                    ...
                                    onChange={(e) =>
                                        setGrantPermissionId(e.target.value === '' ? '' : Number(e.target.value))
                                    }
                                ...
                                <Button onClick={handleGrant} disabled={grantPermissionId === ''}>
                                    Przyznaj
                                </Button>
                            </div>
                        )}
            ...
            <ConfirmDialog
                ...
                message={
                    <>
                        Czy na pewno chcesz usunąć rolę <strong>{deleteTarget?.name}</strong>?
