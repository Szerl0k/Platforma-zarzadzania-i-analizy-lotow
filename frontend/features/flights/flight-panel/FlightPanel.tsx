...
import { formatDuration } from "@/common/utils/flightUtils";
import { getDirectionShort } from "@/common/utils/geoUtils";
import { useFlightPanelData } from "./useFlightPanelData";
import { FlightPreviewMap } from "./FlightPreviewMap";
...
/**
 * Presentational flight details panel. All data orchestration lives in
 * `useFlightPanelData`; metric/GeoJSON shaping lives in `flightMetrics`. This
 * component only renders, so it stays cohesive despite spanning the flights,
 * telemetry, auth and routing concerns.
 */
    ...
    <div className="h-full flex flex-col overflow-hidden w-full bg-surface border-x-2 border-ink">
      ...
      {flightDetails && (
        <div className="p-3 border-b-2 border-ink shrink-0 flex flex-wrap gap-2">
          {isAuthenticated && !hideTrackingButton && (
            <TrackThisFlightButton
