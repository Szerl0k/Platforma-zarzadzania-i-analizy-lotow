...
export function AirportPanel({ airport, selectedAirlineIcaos, onClose, onAirlineToggle, onToggleAll }: AirportPanelProps) {
    ...
    useEffect(() => {
        ...
        getAirportRoutes(airport.icaoCode)
            .then((data) => { if (!cancelled) setRoutesState({ routes: data, loading: false, error: null }); })
            .catch(() => { if (!cancelled) setRoutesState({ routes: [], loading: false, error: 'Nie udało się pobrać tras lotniczych.' }); });
                ...
                <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                        ...
                        <p className="font-mono font-bold text-sm uppercase tracking-widest text-ink truncate">
                            {airport.icaoCode}
                            {airport.iataCode && (
                                <span className="text-ink-muted font-normal"> / {airport.iataCode}</span>
                            )}
                        </p>
            ...
            <div className="flex-1 overflow-y-auto">
                {loading && (
                    <div className="flex flex-col items-center justify-center gap-2 py-10 text-ink-muted">
                        <Spinner size="md" />
                        <p className="font-mono text-xs uppercase tracking-widest">Pobieram trasy…</p>
                    </div>
                )}
                                ...
                                <li key={airline.icaoCode}>
                                    ...
                                    >
                                        ...
                                        <p className={`text-xs font-mono mt-0.5 ${isSelected ? 'text-blue-200' : 'text-ink-muted'}`}>
                                            ...
                                            {' · '}
                                            {destinations.length}{' '}
                                            {destinations.length === 1 ? 'trasa' : destinations.length < 5 ? 'trasy' : 'tras'}
                                        </p>
                                    </button>
                                </li>
                            );
