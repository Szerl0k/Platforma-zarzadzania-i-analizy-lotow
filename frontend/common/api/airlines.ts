...
export async function getAirlineByCode(code: string): Promise<Airline> {
  const { data } = await apiClient.get<Airline>(
    `/airlines/${encodeURIComponent(code)}`,
  );
  return data;
}
...
export async function listAirlines(
  ...
  const { data } = await apiClient.get<AirlineListResponse>("/airlines", {
    params,
  });
  return data;
}
...
export async function updateAirline(
  ...
  const { data } = await apiClient.patch<Airline>(
    `/airlines/${encodeURIComponent(code)}`,
    input,
  );
  return data;
