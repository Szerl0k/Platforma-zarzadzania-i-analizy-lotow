...
export async function getAirportByCode(code: string): Promise<Airport> {
    const { data } = await apiClient.get<Airport>(`/airports/${encodeURIComponent(code)}`);
    return data;
}

export async function listAirports(params: { limit?: number; offset?: number } = {}): Promise<AirportListResponse> {
    const { data } = await apiClient.get<AirportListResponse>('/airports', { params });
    return data;
}
...
export async function updateAirport(code: string, input: AirportUpdateInput): Promise<Airport> {
    const { data } = await apiClient.patch<Airport>(`/airports/${encodeURIComponent(code)}`, input);
    return data;
}
