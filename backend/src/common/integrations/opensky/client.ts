...
export class OpenSkyClient {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      const res = await axios.post(this.authUrl, payload.toString(), {
        headers: {
      ...
      this.handleAxiosError(
        error,
        "Failed to retrieve access token from OpenSky API",
      );
    }
  }
  ...
  public async getAllStateVectors(
    bbox?: BoundingBox,
  ...
  public async getArrivalsByAirport(
    params: FlightTimeWindow,
  ...
  // From OpenSky API documentation:
  // Flights are updated by a batch process at night, i.e., only flights from the previous day or earlier are available using this endpoint.
  public async getDeparturesByAirport(
    params: FlightTimeWindow,
  ): Promise<OpenSkyFlight[]> {
  ...
  // From OpenSky API documentation:
  // Flights are updated by a batch process at night, i.e., only flights from the previous day or earlier are available using this endpoint.
  public async getFlightsByAircraft(
    params: AircraftTimeWindow,
  ): Promise<OpenSkyFlight[]> {
  ...
  public async getFlightTrajectory(
    icao24: string,
    time: number,
  ): Promise<OpenSkyTrackResponse> {
