  ...
  try {
    ...
    try {
      const existingRecords = await AppDataSource.query(
        "SELECT 1 FROM airlines LIMIT 1",
      );

      if (existingRecords && existingRecords.length > 0) {
        console.log(
          "Baseline data detected in the airlines table. Skipping seeding",
        );
        return;
      }
    } catch (error) {
      console.error(
        "Failed to verify table state. Ensure migrations accurately construct the airlines table before seeding.",
      );
      throw error;
    }
