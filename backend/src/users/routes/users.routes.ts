...
router.get('/', authorize('users:write'), async (req: Request, res: Response) => {
...
router.patch('/:id/role', authorize('users:write'), async (req: Request, res: Response) => {
    ...
    if (req.params.id === req.userId) {
        ...
        if (!newPermissions.includes('users:write')) {
            res.status(403).json({ error: 'Cannot remove users:write from your own account' });
            return;
        }
    }
...
router.get('/:id', async (req: Request, res: Response) => {
    ...
    if (!user.profilePublic) {
        const requesterPermissions = req.roleId ? await getPermissionNamesForRole(req.roleId) : [];
