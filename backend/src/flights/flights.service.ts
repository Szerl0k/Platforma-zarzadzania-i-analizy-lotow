...
import { FlightDetailsResponseDTO, CreateFlightDTO, UpdateFlightDTO } from "./flights.dto";
...
export class FlightsService {
   ...
   */
  public async createFlight(data: CreateFlightDTO): Promise<FlightDetailsResponseDTO> {
    ...
    const reloadedFlight = await this.flightsRepository.findById(flight.id, [
      "status", "origin", "origin.city", "origin.city.country", 
      "destination", "destination.city", "destination.city.country", 
      "operatingAirline"
    ]);
   ...
   */
  public async updateFlight(id: string, data: UpdateFlightDTO): Promise<FlightDetailsResponseDTO> {
    const flight = await this.flightsRepository.update(id, data);
    if (!flight) throw new FlightNotFoundError(`Flight with ID ${id} not found`);
  ...
  public async deleteFlight(id: string): Promise<void> {
    ...
    if (!success) throw new FlightNotFoundError(`Flight with ID ${id} not found`);
  }
   ...
   */
  public async getFlightPath(id: string): Promise<{ traveled: any; remaining: any }> {
    const path = await this.flightsRepository.getFlightPath(id);
    if (!path.traveled && !path.remaining) {
      throw new FlightNotFoundError(`Flight path for ID ${id} cannot be constructed (missing origin/destination)`);
    }
    return path;
  }
  ...
  public async getFlightDetailsAndSave(
    ...
    const existingFlight = await this.flightsRepository.findByCallsign(identIcao, [
      "status",
      "origin",
      "origin.city",
      "origin.city.country",
    ...
    if (!response.flights || response.flights.length === 0) {
      throw new FlightNotFoundError(`Could find no active flight details in AeroAPI for ICAO code: ${identIcao}`);
    }
    ...
    const flight = await this.dataSource.transaction(async (manager) => {
      const status = await this.flightsRepository.findOrCreateStatus(flightDetails.status, manager);
      ...
      const flightData: UpdateFlightDTO = {
        ...
        callsign: flightDetails.atc_ident || flightDetails.ident_icao || flightDetails.ident,
      ...
      let flightRecord = await this.flightsRepository.findByFaFlightId(flightDetails.fa_flight_id, [
        "status", "origin", "origin.city", "origin.city.country", 
        "destination", "destination.city", "destination.city.country", 
        "operatingAirline", "codeshares"
      ], manager);

      if (!flightRecord) {
        flightRecord = await this.flightsRepository.create(flightData as CreateFlightDTO, manager);
      } else {
        await this.flightsRepository.update(flightRecord.id, flightData, manager);
      }

      // Handle codeshares
      if (flightDetails.codeshares_iata?.length) {
        await this.flightsRepository.syncCodeshares(flightRecord.id, flightDetails.codeshares_iata, manager);
      }
